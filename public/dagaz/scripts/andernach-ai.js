...
function UndoHistory(ep, castleRights, inCheck, baseEval, hashKeyLow, hashKeyHigh, move50, captured, piece) {
    this.ep = ep;
...
Dagaz.AI.MakeMove = function(move) {
    ...
    g_moveUndoStack[Dagaz.AI.g_moveCount] = new UndoHistory(g_enPassentSquare, g_castleRights, Dagaz.AI.g_inCheck, Dagaz.AI.g_baseEval, Dagaz.AI.g_hashKeyLow, Dagaz.AI.g_hashKeyHigh, Dagaz.AI.g_move50, captured, piece);
    ...
    if ((captured && ((piece & Dagaz.AI.TYPE_MASK) != pieceKing)) || (flags & moveflagPromotion)) {
        var newPiece = piece;
        if (captured && ((piece & Dagaz.AI.TYPE_MASK) != pieceKing)) {
            newPiece &= ~Dagaz.AI.PLAYERS_MASK;
            newPiece |= Dagaz.AI.g_toMove ? Dagaz.AI.colorBlack : Dagaz.AI.colorWhite;
        }
        ...
        if (captured && ((piece & Dagaz.AI.TYPE_MASK) != pieceKing)) {
            Dagaz.AI.g_baseEval -= pieceSquareAdj[newPiece & Dagaz.AI.TYPE_MASK][me ? flipTable[to] : to];
